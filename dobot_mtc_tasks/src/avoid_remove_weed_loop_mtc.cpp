...
namespace avoid_remove_weed_loop
{
    class MTCTaskNode
    {
    public:
    ...
    private:
        ...
        std::vector<Asparagus> asparagus_;
        std::vector<geometry_msgs::msg::PoseStamped> weed_poses_;

    }; // MTCTaskNode

    MTCTaskNode::MTCTaskNode(const rclcpp::NodeOptions& options)
    : node_{std::make_shared<rclcpp::Node>("avoid_remove_weed_loop_mtc", options)}
    {
        service_client_ =
    ...
    bool MTCTaskNode::getSceneFromService()
    {
        ...
        auto request = std::make_shared<dobot_msgs_fb::srv::RemoveWeeds::Request>(); // create empty request
        ...
        // defining weed_poses_

        auto weeds_flat = response->weeds; // flattened array
        weed_poses_.clear();
        ...
        for(size_t i = 0; i < weeds_flat.size(); i+=3){
            ...
            //weed_pose.header.frame_id = "base_link";
    ...
    bool MTCTaskNode::setupPlanningScene()
    {
        ...
        try
        {
            ...
            if(!getSceneFromService()){
                RCLCPP_ERROR(node_->get_logger(), "Could not get scene from service!");
                return false;
            }
            ...
            weed_objects.resize(weed_poses_.size()); // resize

            float height = 0.005;
            float width = 0.002;

            for(size_t i = 0; i < weed_poses_.size(); i++){
                ...
                weed_objects[i].pose = weed_poses_[i].pose;
            ...
            asparagus_objects.resize(asparagus_.size());
            //asparagus_objects.clear();

            for(size_t i = 0; i < asparagus_.size(); i++){

                std::string asparagus_id = "asparagus_" + std::to_string(i);

                asparagus_objects[i].id = asparagus_id;
                asparagus_objects[i].header.frame_id = "base_link";
                asparagus_objects[i].primitives.resize(1);
                asparagus_objects[i].primitives[0].type = shape_msgs::msg::SolidPrimitive::CYLINDER;
                asparagus_objects[i].primitives[0].dimensions = {asparagus_[i].getHeight(), asparagus_[i].getRadius()};

                asparagus_objects[i].pose = asparagus_[i].getPose().pose;

                psi.applyCollisionObject(asparagus_objects[i]);
    ...
    mtc::Task MTCTaskNode::createTask()
    {
        ...
        ****************************************************/
        ...
        /***********************Main Loop*****************************/
        for(unsigned short i = 0; i < weed_poses_.size(); i++){
            std::string weed_id = "weed_" + std::to_string(i);
    ...
    void MTCTaskNode::doTask()
    {
        ...
        auto results = task_.execute(*task_.solutions().front()); // blocking cuntion
...
int main(int argc, char** argv)
{
  ...
  auto mtc_task_node = std::make_shared<avoid_remove_weed_loop::MTCTaskNode>(options);
